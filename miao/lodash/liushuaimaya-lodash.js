var liushuaimaya = {
  ...
  differenceBy: (ary, ...args) => typeof (args[args.length - 1]) == "function" ? ary.filter(x => !(args.slice(0, -1).reduce((res, a) => res.concat(a), [])).map(it => args[args.length - 1](it)).includes(args[args.length - 1](x))) : this.difference(ary, ...args),
  drop: (arr, n = 1) => arr.slice(n),
  dropRight: (arr, n = 1) => arr.slice(0, -n),
  dropRightWhile: (arr, n = 1) => arr.slice(0, -n),
